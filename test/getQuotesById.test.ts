...
describe('getQuotesById API endpoint', () => {
  ...
  it('function, when called successfully with a valid ID, returns the correct movie quotes', async () => {
    ...
    const respData = await getQuotesById(validId, mockHttpClient);
  ...
  errorCases.forEach(({ error, expectedError }) => {
    it(`handles error case: ${error.message} for invalid movie ID`, async () => {
      ...
      await expect(getQuotesById(validId, mockHttpClient)).rejects.toThrow(expectedError);
    });
  });
  ...
  it('handles incorrect movie ID error', async () => {
    ...
    await expect(getQuotesById(incorrectId, mockHttpClient)).rejects.toThrow(ApiError);
  });
  ...
  it('handles incorrect apiKey error', async () => {
    ...
    await expect(getQuotesById(validId, mockHttpClient)).rejects.toThrow(AuthorizationError);
  });
});
